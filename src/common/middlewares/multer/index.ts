...
export const validateFiles = ({
  ...
  customLimits,
  customFileTypes,
}: Types.FileValidatorOptionsProps) => {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    uploadMiddleware(req, res, (err: MulterError | Error | any) => {
      ...
      if (multerErrMsg) {
        return next(new Classes.AppError(multerErrMsg, 400));
      }

      const checkableTypes: Types.MulterType[] = [
        "single",
        "array",
        "any",
        "fields",
      ...
      if (shouldCheckFiles) {
        ...
        if (customErrMsg) {
          return next(new Classes.AppError(customErrMsg, 400));
        }
      }
