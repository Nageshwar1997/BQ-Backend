...
export const uploadHomeVideoController = async (
  ...
  if (!title) {
    throw new Shared.Classes.AppError("Title is required", 400);
  }
  ...
  if (!videoFile) {
    throw new Shared.Classes.AppError("No video file uploaded", 400);
  }

  if (!posterFile) {
    throw new Shared.Classes.AppError("Poster file is required.", 400);
  }
  ...
  if (videoFile.size > MAX_VIDEO_FILE_SIZE) {
    throw new Shared.Classes.AppError("Video file size exceeds 50MB.", 400);
  }

  if (posterFile.size > MAX_IMAGE_FILE_SIZE) {
    throw new Shared.Classes.AppError("Poster image size exceeds 2MB.", 400);
  }
  ...
  if (!video) throw new Shared.Classes.AppError("Video upload failed", 500);
  ...
  if (!poster) throw new Shared.Classes.AppError("Poster upload failed", 500);
  ...
  try {
    ...
    throw new Shared.Classes.AppError("Failed to save video metadata", 500);
  }
};
