...
import { Constants, Models, Utils } from "..";
...
export const uploadHomeVideo = async (
  ...
  if (!CommonConstants.ALLOWED_VIDEO_TYPES.includes(videoFile.mimetype)) {
    throw new Classes.AppError(
      `Invalid video format. Allowed formats: ${CommonConstants.ALLOWED_VIDEO_TYPES.map(
        (t) => t.replace("video/", "")
      ).join(", ")}`,
      400
    );
  ...
  if (!CommonConstants.ALLOWED_IMAGE_TYPES.includes(posterFile.mimetype)) {
    throw new Classes.AppError(
      `Invalid poster format. Allowed formats: ${CommonConstants.ALLOWED_IMAGE_TYPES.map(
        (t) => t.replace("image/", "")
      ).join(", ")}`,
      400
    );
  ...
  if (videoFile.size > CommonConstants.MAX_VIDEO_FILE_SIZE) {
    throw new Classes.AppError("Video file size exceeds 50MB.", 400);
  }

  if (posterFile.size > CommonConstants.MAX_IMAGE_FILE_SIZE) {
    throw new Classes.AppError("Poster image size exceeds 2MB.", 400);
  }

  // Upload video
  const video = await Utils.videoUploader({
    file: videoFile,
  ...
  const poster = await Utils.singleImageUploader({
    file: posterFile,
    folder: `Home/Videos/${title}`,
    cloudinaryConfigOption,
  });
  ...
  let homeVideo;
  try {
    homeVideo = await Models.HomeVideo.create({
      title,
      m3u8Url: video.playback_url,
      originalUrl: video.secure_url,
      public_id: video.public_id,
      duration: Math.round(video.duration),
      posterUrl: CommonUtils.getCloudinaryOptimizedUrl(poster.secure_url),
      user: user?._id,
    });
    ...
    await Promise.all([
      Utils.videoRemover(video.playback_url, cloudinaryConfigOption),
      Utils.singleImageRemover(poster.secure_url, cloudinaryConfigOption),
    ]);
